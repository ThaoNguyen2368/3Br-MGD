...
# 3Br-MGD data.py takes priority (no conflict with FS-GNNTR_repo's data.py — vendored now)
...
# 6. vendor/transformer (vendored from FS-GNNTR)
def test_transformer():
    from vendor.transformer import GNN_prediction
check("vendor.transformer.GNN_prediction", test_transformer)
...
def test_attfpgnn_pkg():
    sys.path.insert(0, os.path.join(_BASELINE_ROOT, 'attfpgnn'))
    import attfpgnn.config as C
    print(f"         AttFPGNN config: EMB={C.EMB_DIM}, GNN={C.GNN_TYPE}")
check("baselines/attfpgnn config", test_attfpgnn_pkg)
