"""
test.py — Meta-testing for FS-GCvTR baseline.

Protocol:
  - Loads shared pre-generated episodes (episodes_seed42_<dataset>.json)
  - For each episode: MAML adaptation on support (k_test=10 steps), evaluate on query
...
from train import (
    ConvTR_dynamic, build_fsgcvtr_model,
    forward_fsgcvtr, compute_fsgcvtr_losses, compute_fsgcvtr_auroc,
    maml_inner_fsgcvtr,
)


def meta_test_step_fsgcvtr(
    gnn, tr, support_samples, query_samples, device,
    gnn_crit, tr_crit, lr_update, n_inner_steps
):
    """
    One FS-GCvTR test episode.
    Returns ROC-AUC (float or NaN).
    """
    support_adapted = [adapt_sample_to_fsgcvtr(s) for s in support_samples if s is not None]
    query_adapted   = [adapt_sample_to_fsgcvtr(q) for q in query_samples   if q is not None]
    ...
    # Evaluate on query
    gnn.eval(); tr.eval()
    auroc = compute_fsgcvtr_auroc(gnn, tr, query_batch)
...
def evaluate(args):
    ...
    for K_shot in args.shots:
        ...
        ckpt_path = args.checkpoint or os.path.join(
            CFG.CHECKPOINT_DIR, f"fsgcvtr_{args.dataset}_{shot_name}_best.pt"
        )
    ...
    results_path = os.path.join(args.output_dir, f"results_fsgcvtr_{args.dataset}.json")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Meta-test FS-GCvTR baseline')
