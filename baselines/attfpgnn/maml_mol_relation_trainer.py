...
class MamlMolRelationTrainer(nn.Module):
    ...
    def adapt_few_shot(self, adapt_data, inner_update_step):
        # torch.enable_grad() đảm bảo inner loop MAML luôn có gradient,
        # ngay cả khi hàm này được gọi từ ngữ cảnh torch.no_grad() (vd: eval loop)
        with torch.enable_grad():
            ...
            for _ in range(inner_update_step):
                ...
                output_weight.data -= self.args.inner_lr * torch.clamp(output_weight.grad, -1.0, 1.0)
                output_bias.data -= self.args.inner_lr * torch.clamp(output_bias.grad, -1.0, 1.0)
                # Reset gradients
                local_optim.zero_grad()
    ...
    def test_step(self):
        ...
        for task_id in range(len(self.test_tasks)):
            ...
            with torch.no_grad():
                ...
                q_preds_np = query_preds[:, 1].detach().cpu().numpy()
                q_labels_np = query_labels.long().detach().cpu().numpy()
                auc = _sklearn_auroc(q_labels_np, q_preds_np)
