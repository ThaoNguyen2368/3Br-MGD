...
class MamlMolRelationModel(nn.Module):
    ...
    def calculate_attented_feats(self, support_feats, query_feats, support_labels, padding_label_dim=32):
        ...
        support_features_actives = support_features_flat[torch.where(support_labels.bool())[0]]
        support_features_inactives = support_features_flat[torch.where(~support_labels.bool())[0]]
        ...
        support_labels = torch.cat([
            torch.ones(activate_size, dtype=torch.long),
            torch.zeros(inactivate_size, dtype=torch.long)
        ]).to(support_features_updated.device)
    ...
    def forward(self, s_data, q_data, train_loss: bool=False, s_label=None, q_pred_adj=False, predictive_val_loss: bool=False, is_functional_call: bool=False):
        ...
        if USE_MIXED_FPS:
            s_fps = torch.from_numpy(np.array([all_fps[smi2id[smi]] for smi in s_data.smiles])).float().to(self.device)
            q_fps = torch.from_numpy(np.array([all_fps[smi2id[smi]] for smi in q_data.smiles])).float().to(self.device)
        ...
        if USE_ATTENTION:
            ...
            if torch.isnan(query_features_updated).any():
                print(f"NaN detected! q_emb:{torch.isnan(q_emb).any()}, q_fps:{torch.isnan(q_fps).any() if USE_MIXED_FPS else 'N/A'}, q_features:{torch.isnan(q_features).any()}, q_updated:{torch.isnan(query_features_updated).any()}")
        else:
            # 分子表示不做任何进一步的更新
    ...
    if distance_metric == "mahalanobis":
        ...
        for c in torch.unique(support_labels):
            ...
            cov_matrix = (
                ...
                + 0.1 * torch.eye(class_features.size(1), class_features.size(1), device=support_feats.device)
            )
            precisions.append(torch.linalg.inv(cov_matrix.double()).float())
